#define DT_DRV_COMPAT zmk_input_processor_move_temp_layer
...
struct temp_layer_state {
    ...
    int64_t move_start_time;
    int32_t move_accumulated;
    bool move_passed;
};
...
/* Movement Helper */
static void update_move_state(struct temp_layer_state *state, const struct temp_layer_config *cfg, struct input_event *event) {
    ...
    // 超窗口/首次触发，重置
    if (state->move_start_time == 0 || now - state->move_start_time > cfg->window_ms) {
        state->move_start_time = now;
        state->move_accumulated = 0;
        state->move_passed = false;
    }

    int32_t move = event->value < 0 ? -event->value : event->value;
    state->move_accumulated += move;

    if (!state->move_passed && state->move_accumulated >= cfg->move_threshold) {
        state->move_passed = true;
        LOG_DBG("Move threshold passed: %d", state->move_accumulated);
    }
}
...
static int temp_layer_handle_event(const struct device *dev, struct input_event *event,
    ...
    update_move_state(&data->state, cfg, event);

    if (!data->state.is_active &&
        data->state.move_passed &&
